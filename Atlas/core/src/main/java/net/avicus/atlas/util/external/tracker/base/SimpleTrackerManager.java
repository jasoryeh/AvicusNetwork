...
public class SimpleTrackerManager implements TrackerManager {
    ...
    private static
    @Nullable
    <T extends Tracker> T findTracker(@Nonnull Map<Class<? extends Tracker>, Tracker> db,
                                      @Nonnull Class<T> search) {
    ...
    private static
    @Nullable
    <T extends Tracker> T setTrackerInDB(@Nonnull Map<Class<? extends Tracker>, Tracker> db,
    ...
    <T extends Tracker> T setTracker(@Nonnull Class<T> trackerClass, @Nullable T tracker) {
        Preconditions.checkNotNull(trackerClass, "tracker class");
        Preconditions.checkArgument(trackerClass.isInstance(tracker),
                "tracker is not an instance of the specified class");
    ...
    @Nullable
    <T extends Tracker> T clearTracker(@Nonnull Class<T> trackerClass,
                                       @Nonnull Class<? extends T> trackerImplClass) {
        Preconditions.checkNotNull(trackerClass, "tracker class");
