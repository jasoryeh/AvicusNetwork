...
import net.avicus.atlas.util.external.tracker.damage.BukkitDamageInfo;
...
public class SimpleResolverManager implements DamageResolverManager {
    ...
    public void register(@Nonnull DamageResolver resolver,
                         @Nonnull Class<? extends DamageResolver>... before) {
        Preconditions.checkNotNull(resolver, "resolver");
            ...
            if (after.contains(loopEntry)) {
                if (before.isEmpty()) {
                    ...
                    throw new IllegalArgumentException(
                            "failed to solve constraints (" + resolver + " could not come before "
                                    + loopEntry.resolver + ") while obeying all other constraints");
                }
            }
        }
