package net.avicus.libraries.tracker.trackers.base.gravity;

import net.avicus.libraries.tracker.base.AbstractTracker;
import net.avicus.libraries.tracker.plugin.TrackerPlugin;
import net.avicus.libraries.tracker.timer.TickTimer;
import net.avicus.libraries.tracker.util.PlayerBlockChecker;
...
public class SimpleGravityKillTracker extends AbstractTracker {
    ...
    private void scheduleCheckFallCancelled(final Fall fall, final long delay) {
        this.plugin.getParent().getServer().getScheduler().runTaskLater(this.plugin.getParent(), new Runnable() {
            public void run() {
    ...
    private void playerBecameAirborne(Fall fall, Location where) {
        if (!fall.isFalling && !this.isSupported(fall)
    ...
    public void playerOnOrOffGround(Player player, boolean onGround) {
        ...
        if (fall != null) {
            ...
            if (brokenBlock != null && this.timer.getTicks() - brokenBlock.time <= MAX_SPLEEF_TIME) {
                fall = new Fall(brokenBlock.breaker, Fall.Cause.SPLEEF, player, Fall.From.FLOOR,
                        brokenBlock.time);
